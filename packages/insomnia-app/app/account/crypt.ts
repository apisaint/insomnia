...
async function _pbkdf2Passphrase(passphrase, salt) {
  if (window.crypto && window.crypto.subtle) {
    ...
    const k = await window.crypto.subtle.importKey('raw', Buffer.from(passphrase, 'utf8'), { name: 'PBKDF2' }, false, ['deriveBits']);
    ...
    const derivedKeyRaw = forge.pkcs5.pbkdf2(passphrase, forge.util.hexToBytes(salt), DEFAULT_PBKDF2_ITERATIONS, DEFAULT_BYTE_LENGTH, forge.md.sha256.create());
