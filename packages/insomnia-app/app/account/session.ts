...
export async function login(rawEmail, rawPassphrase) {
  ...
  const {
    saltKey,
    saltAuth
  } = await _getAuthSalts(email);
  const authSecret = await crypt.deriveKey(passphrase, email, saltKey);
  const secret1 = await crypt.srpGenKey();
  const c = new srp.Client(_getSrpParams(), Buffer.from(saltAuth, 'hex'), Buffer.from(email, 'utf8'), Buffer.from(authSecret, 'hex'), Buffer.from(secret1, 'hex'));
  const srpA = c.computeA().toString('hex');
  const {
    sessionStarterId,
    srpB
  } = await fetch.post('/auth/login-a', {
    srpA,
    email
  }, null);
  ...
  const {
    srpM2
  } = await fetch.post('/auth/login-m1', {
    srpM1,
    sessionStarterId
  }, null);
  ...
  setSessionData(sessionId, accountId, firstName, lastName, email, JSON.parse(symmetricKeyStr), JSON.parse(publicKey), JSON.parse(encPrivateKey));
...
export async function changePasswordWithToken(rawNewPassphrase, confirmationCode) {
  ...
  const newVerifier = srp.computeVerifier(_getSrpParams(), Buffer.from(saltAuth, 'hex'), Buffer.from(newEmail, 'utf8'), Buffer.from(newAuthSecret, 'hex')).toString('hex');
  ...
  return fetch.post('/auth/change-password', {
...
export function setSessionData(sessionId, accountId, firstName, lastName, email, symmetricKey, publicKey, encPrivateKey) {
