...
import { NunjucksParsedTag } from "./utils";
 ...
 */
export function render(text: string, config: {context?: {
    [key: string]: any;
  };path?: string;renderMode?: string;} = {}): Promise<string> {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    nj.renderString(text, templatingContext, (err, result) => {
      if (err) {
        const sanitizedMsg = err.message.replace(/\(unknown path\)\s/, '').replace(/\[Line \d+, Column \d*]/, '').replace(/^\s*Error:\s*/, '').trim();
...
export async function getTagDefinitions(): Promise<Array<NunjucksParsedTag>> {
  ...
  return Object.keys(env.extensions).map(k => env.extensions[k]).filter(ext => !ext.isDeprecated()).sort((a, b) => a.getPriority() > b.getPriority() ? 1 : -1).map(ext => ({
    name: ext.getTag(),
