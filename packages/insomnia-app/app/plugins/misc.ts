...
export async function generateThemeCSS(theme: PluginTheme): Promise<string> {
  const renderedTheme: ThemeInner = await render(theme.theme, theme.theme, null, THROW_ON_ERROR, theme.name);
  const n = theme.name;
  ...
  css += wrapStyles(n, '', getThemeBlockCSS({
    ...renderedTheme,
  ...
  if (renderedTheme.styles) {
    ...
    // Dropdown Menus
    css += wrapStyles(n, '.theme--dropdown__menu', getThemeBlockCSS(styles.dropdown || styles.dialog));

    // Tooltips
    css += wrapStyles(n, '.theme--tooltip', getThemeBlockCSS(styles.tooltip || styles.dialog));

    // Overlay
    css += wrapStyles(n, '.theme--transparent-overlay', getThemeBlockCSS(styles.transparentOverlay));
...
function wrapStyles(theme: string, selector: string, styles: string) {
  ...
  return [`[theme="${theme}"] ${selector}, `, `[subtheme="${theme}"] ${selector ? selector + '--sub' : ''} {`, styles, '}', '', ''].join('\n');
}
...
export async function setFont(settings: {
  [key: string]: any;
}) {
  if (!document) {
    return;
