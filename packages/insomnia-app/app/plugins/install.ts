...
export default async function (lookupName: string): Promise<void> {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const {
        tmpDir
      } = await _installPluginToTmpDir(lookupName);
...
async function _isInsomniaPlugin(lookupName: string): Promise<{
  [key: string]: any;
}> {
  return new Promise((resolve, reject) => {
    console.log('[plugins] Fetching module info from npm');
    childProcess.execFile(escape(process.execPath), ['--no-deprecation', // Because Yarn still uses `new Buffer()`
    escape(_getYarnPath()), 'info', lookupName, '--json'], {
      timeout: 5 * 60 * 1000,
      maxBuffer: 1024 * 1024,
      shell: true,
      env: {
...
async function _installPluginToTmpDir(lookupName: string): Promise<{tmpDir: string;}> {
  return new Promise((resolve, reject) => {
    ...
    childProcess.execFile(escape(process.execPath), ['--no-deprecation', // Because Yarn still uses `new Buffer()`
    escape(_getYarnPath()), 'add', lookupName, '--modules-folder', escape(tmpDir), '--cwd', escape(tmpDir), '--no-lockfile', '--production', '--no-progress'], {
      timeout: 5 * 60 * 1000,
      maxBuffer: 1024 * 1024,
      cwd: tmpDir,
...
export function isDeprecatedDependencies(str: string): boolean {
  ...
  return !!(message && message.includes('no longer maintained') && message.includes('not recommended for usage') && message.includes('upgrade your dependencies'));
}
