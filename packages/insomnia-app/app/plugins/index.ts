...
import { PluginTemplateTag } from "../templating/extensions/index";
import { PluginTheme } from "./misc";
import { RequestGroup } from "../models/request-group";
import { Request } from "../models/request";
import { PluginConfig, PluginConfigMap } from "../models/settings";
...
export type DocumentAction = {
  ...
  action: (context: {
    [key: string]: any;
  }, documents: SpecInfo) => void | Promise<void>;
  label: string;
...
async function _traversePluginPath(pluginMap: {
  [key: string]: any;
}, allPaths: Array<string>, allConfigs: PluginConfigMap) {
...
function _initPlugin(packageJSON: {
  [key: string]: any;
}, module: any, allConfigs: PluginConfigMap, path: string | null | undefined): Plugin {
  const meta = packageJSON.insomnia || {};
  const name = packageJSON.name || meta.name;

  // Find config
  const config: PluginConfig = allConfigs.hasOwnProperty(name) ? allConfigs[name] : _defaultPluginConfig;
