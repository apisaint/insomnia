...
import { changelogUrl, getAppLongName, getAppName, getDefaultAppId, getAppReleaseDate, getAppVersion, isDevelopment, isLinux, isMac, MNEMONIC_SYM } from "../common/constants";
...
export function createWindow() {
  ...
  const appLogo = getDefaultAppId() === APP_ID_INSOMNIA ? 'static/insomnia-core-logo_16x.png' : 'static/insomnia-designer-logo_16x.png';

  let isVisibleOnAnyDisplay = true;
  for (const d of electron.screen.getAllDisplays()) {
    const isVisibleOnDisplay = x >= d.bounds.x && y >= d.bounds.y && x + width <= d.bounds.x + d.bounds.width && y + height <= d.bounds.y + d.bounds.height;
  ...
  const applicationMenu = {
    ...
    }, { type: 'separator' }, { role: 'hide' }, { role: 'hideothers' }, { type: 'separator' }, { label: `${MNEMONIC_SYM}Quit`, accelerator: 'CmdOrCtrl+Q', click: () => app.quit() }]
  };

  const editMenu = {
    label: `${MNEMONIC_SYM}Edit`,
    submenu: [{ label: `${MNEMONIC_SYM}Undo`, accelerator: 'CmdOrCtrl+Z', selector: 'undo:' }, { label: `${MNEMONIC_SYM}Redo`, accelerator: 'Shift+CmdOrCtrl+Z', selector: 'redo:' }, { type: 'separator' }, { label: `Cu${MNEMONIC_SYM}t`, accelerator: 'CmdOrCtrl+X', selector: 'cut:' }, { label: `${MNEMONIC_SYM}Copy`, accelerator: 'CmdOrCtrl+C', selector: 'copy:' }, { label: `${MNEMONIC_SYM}Paste`, accelerator: 'CmdOrCtrl+V', selector: 'paste:' }, {
  ...
  const viewMenu = {
    ...
    submenu: [{ label: `Toggle ${MNEMONIC_SYM}Full Screen`, role: 'togglefullscreen' }, {
  ...
  const windowMenu = {
    ...
    role: 'window',
    submenu: [{ label: `${MNEMONIC_SYM}Minimize`, role: 'minimize' }, ...(isMac() ? [{ label: `${MNEMONIC_SYM}Close`, role: 'close' }] : [])]
  };
  ...
  const aboutMenuClickHandler = async () => {
    ...
    const detail = [`Version: ${getAppLongName()} ${getAppVersion()}`, `Release date: ${getAppReleaseDate()}`, `OS: ${os.type()} ${os.arch()} ${os.release()}`, `Electron: ${process.versions.electron}`, `Node: ${process.versions.node}`, `V8: ${process.versions.v8}`, `Architecture: ${process.arch}`, `node-libcurl: ${Curl.getVersion()}`].join('\n');
