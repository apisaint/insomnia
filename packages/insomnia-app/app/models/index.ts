...
export function all() {
  return [stats, settings, workspace, workspaceMeta, environment, gitRepository, cookieJar, apiSpec, requestGroup, requestGroupMeta, request, requestVersion, requestMeta, response, oAuth2Token, clientCertificate, pluginData, unitTestSuite, unitTestResult, unitTest, protoFile, grpcRequest, grpcRequestMeta];
}
...
export async function initModel<T extends BaseModel>(type: string, ...sources: Array<{
  [key: string]: any;
}>): Promise<T> {
  const model = getModel(type);
