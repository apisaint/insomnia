...
export function newAuth(type: string, oldAuth: RequestAuthentication = {}): RequestAuthentication {
  switch (type) {
    ...
    case AUTH_DIGEST:case AUTH_NTLM:
      return {
        type,
        disabled: oldAuth.disabled || false,
        username: oldAuth.username || '',
...
export function updateMimeType(request: Request, mimeType: string, doCreate: boolean = false, savedBody: RequestBody = {}): Promise<Request> {
  ...
  } else if (mimeType === CONTENT_TYPE_OTHER) {// Leave headers alone
  ...
  if (mimeType === CONTENT_TYPE_FORM_URLENCODED) {
    // Urlencoded
    body = oldBody.params ? newBodyFormUrlEncoded(oldBody.params) : newBodyFormUrlEncoded(deconstructQueryStringToParams(oldBody.text));
  } else if (mimeType === CONTENT_TYPE_FORM_DATA) {
    // Form Data
    body = oldBody.params ? newBodyForm(oldBody.params) : newBodyForm(deconstructQueryStringToParams(oldBody.text));
