...
export async function migrate(doc: {
  [key: string]: any;
}) {
...
async function _findRecentForRequest(requestId: string, environmentId: string | null, limit: number): Promise<Array<Response>> {
  const query: {
    [key: string]: any;
  } = {
    parentId: requestId
...
export async function getLatestForRequest(requestId: string, environmentId: string | null): Promise<Response | null> {
...
export async function create(patch: {
  ...
  if ((await models.settings.getOrCreate()).filterResponsesByEnv && patch.hasOwnProperty('environmentId')) {
...
export function getBodyStream<T>(response: {
  [key: string]: any;
}, readFailureValue: T | null | undefined): Readable | null | T {
  return getBodyStreamFromPath(response.bodyPath || '', response.bodyCompression, readFailureValue);
}

export function getBodyBuffer<T>(response: {
  [key: string]: any;
}, readFailureValue: T | null | undefined): Buffer | T | null {
  return getBodyBufferFromPath(response.bodyPath || '', response.bodyCompression, readFailureValue);
}

export function getTimeline(response: {
  [key: string]: any;
}): Array<ResponseTimelineEntry> {
  return getTimelineFromPath(response.timelinePath || '');
}

function getBodyStreamFromPath<T>(bodyPath: string, compression: string | null, readFailureValue: T | null | undefined): Readable | null | T {
...
function getBodyBufferFromPath<T>(bodyPath: string, compression: string | null, readFailureValue: T | null | undefined): Buffer | T | null {
  // No body, so return empty Buffer
  if (!bodyPath) {
...
async function migrateBodyToFileSystem(doc: {
  [key: string]: any;
}) {
...
function migrateBodyCompression(doc: {
  [key: string]: any;
}) {
...
async function migrateTimelineToFileSystem(doc: {
  [key: string]: any;
}) {
...
export async function cleanDeletedResponses() {
  ...
  for (const filePath of files) {
    ...
    } catch (err) {// Just keep going, doesn't matter
    }
  }
}