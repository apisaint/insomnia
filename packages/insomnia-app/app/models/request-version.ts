...
const FIELDS_TO_IGNORE = ['_id', 'type', 'created', 'modified', 'metaSortKey', 'description', 'parentId', 'name'];
...
export async function create(request: Request): Promise<RequestVersion> {
  ...
  const latestRequest = latestRequestVersion ? decompressObject(latestRequestVersion.compressedRequest) : null;
