...
export async function init(types: Array<string>, config: {
  [key: string]: any;
} = {}, forceReset: boolean = false, consoleLog: () => void = console.log) {
  if (forceReset) {
    changeListeners = [];
...
export const bufferChanges = database.bufferChanges = async function (millis: number = 1000): Promise<number> {
...
export const flushChangesAsync = database.flushChangesAsync = async function (fake: boolean = false) {
  process.nextTick(async () => {
    await flushChanges(0, fake);
  });
};

export const flushChanges = database.flushChanges = async function (id: number = 0, fake: boolean = false) {
...
export const getMostRecentlyModified = database.getMostRecentlyModified = async function <T>(type: string, query: {
  [key: string]: any;
} = {}): Promise<T | null> {
...
export const findMostRecentlyModified = database.findMostRecentlyModified = async function <T>(type: string, query: {
  [key: string]: any;
} = {}, limit: number | null = null): Promise<Array<T>> {
...
export const find = database.find = async function <T extends BaseModel>(type: string, query: {
  [key: string]: any;
} = {}, sort: {
  [key: string]: any;
} = { created: 1 }): Promise<Array<T>> {
...
export const all = database.all = async function <T extends BaseModel>(type: string): Promise<Array<T>> {
  if (db._empty) return _send('all', ...arguments);

  return database.find(type);
};

export const getWhere = database.getWhere = async function <T extends BaseModel>(type: string, query: {
  [key: string]: any;
}): Promise<T | null> {
...
export const get = database.get = async function <T extends BaseModel>(type: string, id: string): Promise<T | null> {
...
export const count = database.count = async function (type: string, query: {
  [key: string]: any;
} = {}): Promise<number> {
...
export const upsert = database.upsert = async function (doc: BaseModel, fromSync: boolean = false): Promise<BaseModel> {
...
export const insert = database.insert = async function <T extends BaseModel>(doc: T, fromSync: boolean = false, initializeModel: boolean = true): Promise<T> {
...
export const update = database.update = async function <T extends BaseModel>(doc: T, fromSync: boolean = false): Promise<T> {
...
export const remove = database.remove = async function <T extends BaseModel>(doc: T, fromSync: boolean = false): Promise<void> {
...
export const unsafeRemove = database.unsafeRemove = async function <T extends BaseModel>(doc: T, fromSync: boolean = false): Promise<void> {
...
export const removeWhere = database.removeWhere = async function (type: string, query: {
  [key: string]: any;
}): Promise<void> {
...
export async function docUpdate<T extends BaseModel>(originalDoc: T, ...patches: Array<{
  [key: string]: any;
}>): Promise<T> {
  // No need to re-initialize the model during update; originalDoc will be in a valid state by virtue of loading
  const doc = await models.initModel(originalDoc.type, originalDoc, // NOTE: This is before `patch` because we want `patch.modified` to win if it has it
  { modified: Date.now() }, ...patches);

  return database.update(doc);
}

export async function docCreate<T extends BaseModel>(type: string, ...patches: Array<{
  [key: string]: any;
}>): Promise<T> {
  const doc = await models.initModel(type, ...patches, // Fields that the user can't touch
  { type: type });
...
export const withDescendants = database.withDescendants = async function (doc: BaseModel | null, stopType: string | null = null): Promise<Array<BaseModel>> {
...
export const withAncestors = database.withAncestors = async function (doc: BaseModel | null, types: Array<string> = allTypes()): Promise<Array<BaseModel>> {
...
export const duplicate = database.duplicate = async function <T extends BaseModel>(originalDoc: T, patch: {
  [key: string]: any;
} = {}): Promise<T> {
  ...
  async function next<T extends BaseModel>(docToCopy: T, patch: {
    [key: string]: any;
  }): Promise<T> {
