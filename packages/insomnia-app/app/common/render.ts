...
import { GrpcRequest, GrpcRequestBody } from "../models/grpc-request";
...
export async function buildRenderContext(ancestors: Array<BaseModel> | null, rootEnvironment: Environment | null, subEnvironment: Environment | null, baseContext: {
  [key: string]: any;
} = {}): {
  [key: string]: any;
} {
  ...
  async function renderSubContext(subObject: {
    [key: string]: any;
  }, subContext: {
    [key: string]: any;
  }): Promise<any> {
    ...
    for (const key of keys) {
      ...
      if (Object.prototype.toString.call(subObject[key]) === '[object String]') {
        ...
        if (isSelfRecursive) {
          ...
          // render it first
          subContext[key] = await render(subObject[key], subContext, // Only render with key being overwritten
          null, KEEP_ON_ERROR, 'Environment');
        } else {
  ...
  for (let i = 0; i < 3; i++) {
    for (const key of keys) {
      ...
      const renderResult = await render(finalRenderContext[key], finalRenderContext, null, KEEP_ON_ERROR, 'Environment');
 ...
 * @return {Promise.<*>}
 */
export async function render<T>(obj: T, context: {
  [key: string]: any;
} = {}, blacklistPathRegex: RegExp | null = null, errorMode: string = THROW_ON_ERROR, name: string = ''): Promise<T> {
  ...
  async function next(x: any, path: string, first: boolean = false): Promise<any> {
    ...
    if (asStr === '[object Date]' || asStr === '[object RegExp]' || asStr === '[object Error]' || asStr === '[object Boolean]' || asStr === '[object Number]' || asStr === '[object Null]' || asStr === '[object Undefined]') {// Do nothing to these types
...
export async function getRenderContext(request: Request | GrpcRequest, environmentId: string | null, ancestors: Array<BaseModel> | null = null, purpose: RenderPurpose | null = null, extraInfo: ExtraRenderInfo | null = null): Promise<{
  [key: string]: any;
}> {
  if (!ancestors) {
  ...
  if (ancestors) {
    for (let idx = 0; idx < ancestors.length; idx++) {
      ...
      if (isRequestGroup(ancestor) && ancestor.hasOwnProperty('environment') && ancestor.hasOwnProperty('name')) {
...
export async function getRenderedGrpcRequest(request: GrpcRequest, environmentId: string | null, purpose?: RenderPurpose, extraInfo?: ExtraRenderInfo, skipBody?: boolean): Promise<{request: RenderedGrpcRequest;context: {
    [key: string]: any;
  };}> {
  const renderContext = await getRenderContext(request, environmentId, null, purpose, extraInfo || null);
  ...
  const renderedRequest: RenderedGrpcRequest = await render(request, renderContext, ignorePathRegex);
...
export async function getRenderedGrpcRequestMessage(request: GrpcRequest, environmentId: string | null, purpose?: RenderPurpose, extraInfo?: ExtraRenderInfo): Promise<RenderedGrpcRequestBody> {
  const renderContext = await getRenderContext(request, environmentId, null, purpose, extraInfo || null);
...
export async function getRenderedRequestAndContext(request: Request, environmentId: string | null, purpose?: RenderPurpose, extraInfo?: ExtraRenderInfo): Promise<{request: RenderedRequest;context: {
    [key: string]: any;
  };}> {
  ...
  const renderContext = await getRenderContext(request, environmentId, ancestors, purpose, extraInfo || null);
  ...
  // Render all request properties
  const renderResult = await render({ _request: request, _cookieJar: cookieJar }, renderContext, request.settingDisableRenderRequestBody ? /^body.*/ : null);
...
function _getOrderedEnvironmentKeys(finalRenderContext: {
  [key: string]: any;
}): Array<string> {
...
async function _getRequestAncestors(request: Request | GrpcRequest): Promise<Array<BaseModel>> {
  return await db.withAncestors(request, [models.request.type, models.grpcRequest.type, models.requestGroup.type, models.workspace.type]);
}