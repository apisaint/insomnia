...
import { GrpcMethodDefinition, GrpcMethodType } from "../network/grpc/method";
...
export const getShortGrpcPath = ({
  ...
  serviceName,
  methodName
}: GrpcPathSegments, fullPath: string): string => {
...
export const groupGrpcMethodsByPackage = (methods: Array<GrpcMethodDefinition>): GroupedGrpcMethodInfo => groupBy(methods.map(getMethodInfo), m => m.segments.packageName || NO_PACKAGE_KEY);