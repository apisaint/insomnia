...
export type ImportResult = {
  ...
  error: Error | null;
  summary: {
    [key: string]: Array<BaseModel>;
  };
};

export async function importUri(getWorkspaceId: () => Promise<string | null>, uri: string): Promise<ImportResult> {
  let rawText;
  ...
  const statements = Object.keys(summary).map(type => {
...
export async function importRaw(getWorkspaceId: () => Promise<string | null>, rawContent: string): Promise<ImportResult> {
  let results;
  try {
  ...
  for (const resource of data.resources) {
    ...
    if (isRequest(model) && resource.body && typeof resource.body.text === 'string' && typeof resource.url === 'string' && resource.body.text.includes('"query"') && resource.url.includes('graphql')) {
      resource.body.mimeType = CONTENT_TYPE_GRAPHQL;
    }

    // Try adding Content-Type JSON if no Content-Type exists
    if (isRequest(model) && resource.body && typeof resource.body.text === 'string' && Array.isArray(resource.headers) && !resource.headers.find(h => h.name.toLowerCase() === 'content-type')) {
      try {
...
export async function exportWorkspacesHAR(parentDoc: BaseModel | null = null, includePrivateDocs: boolean = false): Promise<string> {
...
export async function exportRequestsHAR(requests: Array<BaseModel>, includePrivateDocs: boolean = false): Promise<string> {
  const workspaces: Array<BaseModel> = [];
  const mapRequestIdToWorkspace: {
    [key: string]: any;
  } = {};
  ...
  for (const request of requests) {
    const ancestors: Array<BaseModel> = await db.withAncestors(request, [models.workspace.type, models.requestGroup.type]);
  ...
  const mapWorkspaceIdToEnvironmentId: {
    [key: string]: any;
  } = {};
  ...
  requests = requests.sort((a: {
    [key: string]: any;
  }, b: {
    [key: string]: any;
  }) => a.metaSortKey < b.metaSortKey ? -1 : 1);
  const harRequests: Array<{
    [key: string]: any;
  }> = [];
  for (const request of requests) {
...
export async function exportWorkspacesData(parentDoc: BaseModel | null, includePrivateDocs: boolean, format: "json" | "yaml"): Promise<string> {
  const docs: Array<BaseModel> = await getDocWithDescendants(parentDoc, includePrivateDocs);
...
export async function exportRequestsData(requests: Array<BaseModel>, includePrivateDocs: boolean, format: "json" | "yaml"): Promise<string> {
  const data = {
    _type: 'export',
  ...
  for (const workspace of workspaces) {
    const descendants: Array<BaseModel> = (await db.withDescendants(workspace)).filter(d => {
      ...
      return (d.type === models.cookieJar.type || d.type === models.environment.type || d.type === models.apiSpec.type || d.type === models.unitTestSuite.type || d.type === models.unitTest.type || isProtoFile(d));
    });
  ...
  data.resources = docs.filter(d => {
    ...
    if (!(d.type === models.unitTestSuite.type || d.type === models.unitTest.type || isRequest(d) || isGrpcRequest(d) || isRequestGroup(d) || isProtoFile(d) || d.type === models.workspace.type || d.type === models.cookieJar.type || d.type === models.environment.type || d.type === models.apiSpec.type)) {
      return false;
    }
    // BaseModel doesn't have isPrivate, so cast it first.
    return !(d as {
      [key: string]: any;
    }).isPrivate || includePrivateDocs;
  }).map((d: {
    [key: string]: any;
  }) => {
...
async function getDocWithDescendants(parentDoc: BaseModel | null = null, includePrivateDocs: boolean = false): Promise<Array<BaseModel>> {
  const docs = await db.withDescendants(parentDoc);
  return docs.filter(d => // Don't include if private, except if we want to
