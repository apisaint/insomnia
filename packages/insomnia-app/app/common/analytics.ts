...
import { getAppId, getAppName, getAppPlatform, getAppVersion, getGoogleAnalyticsId, getGoogleAnalyticsLocation, isDevelopment } from "./constants";
...
export function trackEvent(category: string, action: string, label: string | null | undefined, value: string | null | undefined) {
  process.nextTick(async () => {
    await _trackEvent(true, category, action, label, value);
  });
}

export function trackNonInteractiveEvent(category: string, action: string, label: string | null | undefined, value: string | null | undefined) {
  process.nextTick(async () => {
 ...
 * @param value
 */
export function trackNonInteractiveEventQueueable(category: string, action: string, label: string | null | undefined, value: string | null | undefined) {
  process.nextTick(async () => {
...
export async function _trackEvent(interactive: boolean, category: string, action: string, label: string | null | undefined, value: string | null | undefined, queuable: boolean | null | undefined) {
  const prefix = interactive ? '[ga] Event' : '[ga] Non-interactive';
  console.log(prefix, [category, action, label, value].filter(Boolean).join(', '));

  const params = [{ name: KEY_HIT_TYPE, value: 'event' }, { name: KEY_EVENT_CATEGORY, value: category }, { name: KEY_EVENT_ACTION, value: action }];
...
async function _getDefaultParams(): Promise<Array<RequestParameter>> {
  ...
  const ua = String((window?.navigator?.userAgent)).replace(new RegExp(`${getAppId()}\\/\\d+\\.\\d+\\.\\d+ `), '').replace(/Electron\/\d+\.\d+\.\d+ /, '');

  const params = [{ name: KEY_VERSION, value: '1' }, { name: KEY_TRACKING_ID, value: getGoogleAnalyticsId() }, { name: KEY_CLIENT_ID, value: deviceId }, { name: KEY_USER_AGENT, value: ua }, { name: KEY_LOCATION, value: getGoogleAnalyticsLocation() + _currentLocationPath }, { name: KEY_SCREEN_RESOLUTION, value: getScreenResolution() }, { name: KEY_USER_LANGUAGE, value: getUserLanguage() }, { name: KEY_TITLE, value: `${getAppId()}:${getAppVersion()}` }, { name: KEY_CUSTOM_DIMENSION_PREFIX + DIMENSION_PLATFORM, value: getAppPlatform() }, { name: KEY_CUSTOM_DIMENSION_PREFIX + DIMENSION_VERSION, value: getAppVersion() }, { name: KEY_ANONYMIZE_IP, value: '1' }, { name: KEY_APPLICATION_NAME, value: getAppName() }, { name: KEY_APPLICATION_ID, value: getAppId() }, { name: KEY_APPLICATION_VERSION, value: getAppVersion() }];
...
async function _sendToGoogle(params: Array<RequestParameter>, queueable: boolean) {
  ...
  const baseUrl = isDevelopment() ? 'https://www.google-analytics.com/debug/collect' : 'https://www.google-analytics.com/collect';
