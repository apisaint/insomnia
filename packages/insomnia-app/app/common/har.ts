...
export async function exportHar(exportRequests: Array<ExportRequest>): Promise<Har> {
  ...
  for (const exportRequest of exportRequests) {
    ...
    const response: ResponseModel | null = await models.response.getLatestForRequest(exportRequest.requestId, exportRequest.environmentId || null);
...
export async function exportHarWithRequest(request: Request, environmentId: string | null, addContentLength: boolean = false): Promise<HarRequest | null> {
  try {
...
export async function exportHarWithRenderedRequest(renderedRequest: RenderedRequest, addContentLength: boolean = false): Promise<HarRequest> {
  ...
  if (addContentLength) {
    const hasContentLengthHeader = misc.filterHeaders(renderedRequest.headers, 'Content-Length').length > 0;
