...
import { syncCreateResourceGroup, syncFixDupes, syncGetResourceGroup, syncPull, syncPush, syncResetData } from "./network";
...
export async function init() {
  ...
  db.onChange(async changes => {
    ...
    const sortedChanges = changes.sort(([event, doc, fromSync]) => doc.type === models.workspace.type ? 1 : -1);
...
export async function fixDuplicateResourceGroups() {
  ...
  for (const workspace of workspaces) {
    ...
    const {
      deleteResourceGroupIds
    } = await syncFixDupes(ids);
...
export async function push(resourceGroupId = null) {
  ...
  for (const serverResource of conflicts) {
    const localResource = await store.getResourceByDocId(serverResource.id, serverResource.resourceGroupId);
...
export async function pull(resourceGroupId = null, createMissingResources = true) {
  ...
  for (const serverResource of createdResources) {
    ...
    if (serverResource.type === models.clientCertificate.type && config.syncDisableClientCertificates) {
  ...
  for (const serverResource of updatedResources) {
    try {
      ...
      // Update local resource
      const resource = await store.getResourceByDocId(serverResource.id, serverResource.resourceGroupId);
...
async function _getResourceGroupSymmetricKey(resourceGroupId) {
  ...
  if (!key) {
    ...
    const symmetricKeyStr = crypt.decryptRSAWithJWK(accountPrivateKey, resourceGroup.encSymmetricKey);
...
export async function decryptDoc(resourceGroupId, messageJSON) {
  ...
  } catch (err) {// It's not compressed (legacy), which is okay for now
  }
...
export async function getOrCreateAllActiveResources(resourceGroupId = null) {
  ...
  const modelTypes = Object.keys(WHITE_LIST).sort((a, b) => a.type === models.workspace.type ? 1 : -1);
    ...
    for (const doc of await db.all(type)) {
      ...
      if (!resource) {
        try {
          ...
          created++;
        } catch (e) {// logger.warn(`Failed to create resource for ${doc._id} ${e}`, {doc});
        }
      }
    }
  }
