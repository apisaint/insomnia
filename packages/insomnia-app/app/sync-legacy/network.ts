...
export async function syncCreateResourceGroup(parentResourceId, name, encSymmetricKey) {
  return fetch.post('/api/resource_groups', {
    parentResourceId,
    name,
    encSymmetricKey
...
export async function unshareWithAllTeams(resourceGroupId) {
  return fetch.put(`/api/resource_groups/${resourceGroupId}/unshare`, null, session.getCurrentSessionId());
}

export async function shareWithTeam(resourceGroupId, teamId) {
  // Ask the server what we need to do to invite the member
  const instructions = await fetch.post(`/api/resource_groups/${resourceGroupId}/share-a`, {
    teamId
  }, session.getCurrentSessionId());

  const privateKeyJWK = session.getPrivateKey();
  const resourceGroupSymmetricKey = crypt.decryptRSAWithJWK(privateKeyJWK, instructions.encSymmetricKey);
  ...
  await fetch.post(`/api/resource_groups/${resourceGroupId}/share-b`, {
    teamId,
    keys: newKeys
  }, session.getCurrentSessionId());
}