...
export async function buildMultipart(params: Array<RequestBodyParameter>) {
  return new Promise(async (resolve: (...args: Array<any>) => any, reject: (...args: Array<any>) => any) => {
    ...
    for (const param of params) {
      ...
      if (param.type === 'file' && param.fileName) {
        ...
        addString('Content-Disposition: form-data; ' + `name="${name.replace(/"/g, '\\"')}"; ` + `filename="${path.basename(fileName).replace(/"/g, '\\"')}"`);
        addString(lineBreak);
