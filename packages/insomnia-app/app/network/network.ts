...
import { ResponseHeader, ResponseTimelineEntry } from "../models/response";
import { Request, RequestHeader } from "../models/request";
import { Workspace } from "../models/workspace";
import { Settings } from "../models/settings";
import { ExtraRenderInfo, RenderedRequest } from "../common/render";
import { getRenderedRequestAndContext, RENDER_PURPOSE_NO_RENDER, RENDER_PURPOSE_SEND } from "../common/render";
import mkdirp from "mkdirp";
import crypto from "crypto";
import clone from "clone";
import { parse as urlParse, resolve as urlResolve } from "url";
import { Curl, CurlAuth, CurlCode, CurlInfoDebug, CurlFeature, CurlNetrc, CurlHttpVersion } from "node-libcurl";
import { join as pathJoin } from "path";
import * as uuid from "uuid";
import * as models from "../models";
import { AUTH_AWS_IAM, AUTH_DIGEST, AUTH_NETRC, AUTH_NTLM, CONTENT_TYPE_FORM_DATA, CONTENT_TYPE_FORM_URLENCODED, getAppVersion, getTempDir, HttpVersions, STATUS_CODE_PLUGIN_ERROR } from "../common/constants";
import { delay, describeByteSize, getContentTypeHeader, getDataDirectory, getHostHeader, getLocationHeader, getSetCookieHeaders, hasAcceptEncodingHeader, hasAcceptHeader, hasAuthHeader, hasContentTypeHeader, hasUserAgentHeader, waitForStreamToFinish } from "../common/misc";
...
export async function _actuallySend(renderedRequest: RenderedRequest, renderContext: {
  [key: string]: any;
}, workspace: Workspace, settings: Settings, environment: Environment | null): Promise<ResponsePatch> {
    ...
    try {
      ...
      setOpt(Curl.option.DEBUGFUNCTION, (infoType: string, contentBuffer: Buffer) => {
        ...
        if (infoType === CurlInfoDebug.DataOut) {
          if (contentBuffer.length === 0) {// Sometimes this happens, but I'm not sure why. Just ignore it.
      ...
      if (renderedRequest.settingSendCookies) {
        ...
        for (const cookie of cookies) {
          ...
          setOpt(Curl.option.COOKIELIST, [cookie.httpOnly ? `#HttpOnly_${cookie.domain}` : cookie.domain, cookie.hostOnly ? 'FALSE' : 'TRUE', cookie.path, cookie.secure ? 'TRUE' : 'FALSE', expiresTimestamp, cookie.key, cookie.value].join('\t'));
        }

        for (const {
          name,
          value
        } of renderedRequest.cookies) {
      ...
      if (renderedRequest.body.mimeType === CONTENT_TYPE_FORM_URLENCODED) {
        ...
        const fn = () => {
          fs.closeSync(fd);
          fs.unlink(multipartBodyPath, () => {// Pass
          });
        };
      ...
      if (!hasAuthHeader(headers) && !renderedRequest.authentication.disabled) {
        if (renderedRequest.authentication.type === AUTH_DIGEST) {
          ...
          const extraHeaders = _getAwsAuthHeaders(credentials, headers, requestBody || '', finalUrl, renderedRequest.method, authentication.region || '', authentication.service || '');
...
export async function sendWithSettings(requestId: string, requestPatch: {
  [key: string]: any;
  ...
  const ancestors = await db.withAncestors(request, [models.request.type, models.requestGroup.type, models.workspace.type]);
  ...
  let renderResult: {request: RenderedRequest;context: {
      [key: string]: any;
    };};
  try {
...
export async function send(requestId: string, environmentId?: string, extraInfo?: ExtraRenderInfo): Promise<ResponsePatch> {
  ...
  const renderResult = await getRenderedRequestAndContext(request, environmentId || null, RENDER_PURPOSE_SEND, extraInfo);
  ...
  const response = await _actuallySend(renderedRequest, renderedContextBeforePlugins, workspace, settings, environment);

  console.log(response.error ? `[network] Response failed req=${requestId} err=${response.error || 'n/a'}` : `[network] Response succeeded req=${requestId} status=${response.statusCode || '?'}`);

  return response;
}

async function _applyRequestPluginHooks(renderedRequest: RenderedRequest, renderedContext: {
  [key: string]: any;
  ...
  for (const {
    ...
    const context = {
      ...(pluginContexts.app.init(RENDER_PURPOSE_NO_RENDER) as {
        [key: string]: any;
      }),
      ...(pluginContexts.data.init() as {
        [key: string]: any;
      }),
      ...(pluginContexts.store.init(plugin) as {
        [key: string]: any;
      }),
      ...(pluginContexts.request.init(newRenderedRequest, renderedContext) as {
        [key: string]: any;
      }),
      ...(pluginContexts.network.init(renderedContext.getEnvironmentId()) as {
        [key: string]: any;
      })
    };
...
async function _applyResponsePluginHooks(response: ResponsePatch, renderedRequest: RenderedRequest, renderedContext: {
  [key: string]: any;
}): Promise<ResponsePatch> {
  ...
  for (const {
    plugin,
    hook
  } of await plugins.getResponseHooks()) {
    const context = {
      ...(pluginContexts.app.init(RENDER_PURPOSE_NO_RENDER) as {
        [key: string]: any;
      }),
      ...
      ...(pluginContexts.response.init(newResponse) as {
        [key: string]: any;
      }),
      ...(pluginContexts.request.init(newRequest, renderedContext, true) as {
        [key: string]: any;
      }),
      ...(pluginContexts.network.init(renderedContext.getEnvironmentId()) as {
        [key: string]: any;
...
export function _parseHeaders(buffer: Buffer): Array<{
  headers: Array<ResponseHeader>;
  version: string;
  code: number;
  reason: string;
...
export function _getAwsAuthHeaders(credentials: {
  accessKeyId: string;
  secretAccessKey: string;
  sessionToken: string;
}, headers: Array<RequestHeader>, body: string, url: string, method: string, region?: string, service?: string): Array<{name: string;value: string;description?: string;disabled?: boolean;}> {
  ...
  return Object.keys(signature.headers).filter(name => name !== 'content-type') // Don't add this because we already have it
  .map(name => ({
    name,
